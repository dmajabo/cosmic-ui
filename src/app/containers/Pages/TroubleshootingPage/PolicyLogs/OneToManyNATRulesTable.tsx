...
interface OneToManyNATRuleTableProps {
  readonly resourceData: PortRule[];
  readonly styles?: React.CSSProperties;
}

export const OneToManyNATRuleTable: React.FC<OneToManyNATRuleTableProps> = ({ resourceData, styles }) => {
            ...
            {resourceData && resourceData.length
              ? resourceData.map(resource => (
                  <TableRow hover tabIndex={-1} key={`tableRow_${resource.extId}`} className={classes.row}>
                    ...
                    <TableCell className={classes.tableCell}>{resource.localIp}</TableCell>
