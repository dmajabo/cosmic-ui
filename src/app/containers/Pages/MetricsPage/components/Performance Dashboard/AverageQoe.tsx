  ...
  return (
    <div className={classes.flexContainer}>
      <div className={classes.averageQoeText}>
        ...
        <span style={{ color: getColor(PACKET_LOSS_HEATMAP_LEGEND, Number(packetLoss)) }} className={classes.qoeValueText}>{`${
          isNaN(Number(packetLoss)) ? 'NaN' : `${Number(packetLoss) > 0 ? Number(packetLoss).toFixed(2) : Number(packetLoss)}%`
        }`}</span>
        <span>Latency:</span>
