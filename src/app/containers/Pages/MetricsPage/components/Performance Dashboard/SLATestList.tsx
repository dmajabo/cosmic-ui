...
const getDefaultSelectedTestId = (tests: FinalTableData[], selectedRows: FinalTableData[]): Record<string, boolean> => {
  ...
  const selectedTestIds = selectedRows.map(test => test.id);
  if (isEmpty(testsWithIndex)) {
    return {};
  }
  if (isEmpty(selectedTestIds)) {
    const selectedRowsObject: Record<string, boolean> = {};
    selectedRowsObject[0] = true;
...
const getFilteredTableData = (tableData: FinalTableData[], tagsOptions: FilterTagsOption[], siteSegmentsOptions: IMapped_Segment[]) => {
  ...
  if (isEmpty(selectedTags)) {
    if (isEmpty(selectedSites)) {
      ...
      return tableData.filter(test => test.segments.some(segment => selectedSites.includes(segment.id)));
    }
  } else {
    if (isEmpty(selectedSites)) {
      return tableData.filter(test => test.tags.some(tag => selectedTags.includes(tag.value)));
    } else {
...
export const SLATestList: React.FC<SLATestListProps> = ({ networks, devices }) => {
  ...
  //   const twoSelectedRows = value.filter((_test, index) => index <= 1);
  //   setSelectedRows(twoSelectedRows);
  //   localStorage.setItem(LOCAL_STORAGE_SELECTED_TESTS_KEY, JSON.stringify(twoSelectedRows));
  // };

  // const onSlaTestPanelOpen = () => {
  //   getSLATests(true);
  //   setIsSlaTestPanelOpen(true);
  // };
  ...
  //     },
  //     {
  //       Header: isEmpty(tagsOptions) ? '' : <FilterHeader title="TAGS" onItemClick={onTagClick} tagOptions={tagsOptions} />,
  ...
  //     },
  //     {
  //       Header: isEmpty(sitesSegmentsOptions) ? '' : <FilterHeader title="SITES" onItemClick={onSegmentClick} segmentOptions={sitesSegmentsOptions} />,
        ...
        <div className={classes.pageComponentTitleContainer}>
          ...
          <div>
            <MatSelect
              ...
              label="Show"
              labelStyles={{ margin: 'auto 0 auto 0' }}
