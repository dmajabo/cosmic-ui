...
const RegionNode: React.FC<Props> = (props: Props) => {
  ...
  return (
    <TransitionContainer stateIn={visible}>
      <g id={`${NODES_CONSTANTS.REGION.type}${props.dataItem.uiId}`} className="topologyNode" transform={`translate(${pos.x}, ${pos.y})`} data-type={NODES_CONSTANTS.REGION.type}>
        <g style={{ cursor: 'pointer' }} pointerEvents="all" onMouseEnter={showExpandIcon} onMouseLeave={hideExpandIcon}>
          <rect
            fill={NODES_CONSTANTS.REGION.bgColor}
            width={NODES_CONSTANTS.REGION.width}
            height={NODES_CONSTANTS.REGION.height}
            rx={NODES_CONSTANTS.REGION.borderRadius}
            ry={NODES_CONSTANTS.REGION.borderRadius}
            pointerEvents="all"
          />
          <use pointerEvents="none" href={`#${NODES_CONSTANTS.REGION.type}`} x={NODES_CONSTANTS.REGION.iconOffsetX} y={NODES_CONSTANTS.REGION.iconOffsetY} />
          <NodeCounter label={`${12} VPC`} stylesObj={NODES_CONSTANTS.REGION.countStyles} />
          <CollapseExpandButton
            id={props.dataItem.id}
            isCollapse={props.dataItem.collapsed}
            onClick={() => {}}
            x={NODES_CONSTANTS.REGION.width - NODES_CONSTANTS.COLLAPSE_EXPAND.r}
            y={NODES_CONSTANTS.REGION.height / 2 - NODES_CONSTANTS.COLLAPSE_EXPAND.r}
          />
        </g>
        <NodeName label={props.dataItem.name} stylesObj={NODES_CONSTANTS.REGION.labelStyles} />
      </g>
    </TransitionContainer>
  );
};
