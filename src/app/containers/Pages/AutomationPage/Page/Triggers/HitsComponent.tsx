...
const HitsComponent: React.FC<Props> = (props: Props) => {
  ...
  return (
    <NestedTableWrapper style={{ height: rows && rows.length ? 'auto' : '300px', minHeight: rows && rows.length ? '100px' : '300px' }}>
      <DataTable className="tableSM" emptyMessage="No data" dataKey="id" value={rows} responsiveLayout="scroll">
        ...
        <Column style={{ minWidth: NestedTriggerGridColumns.state.width }} field={NestedTriggerGridColumns.state.field} header={NestedTriggerGridColumns.state.label} body={stateBodyTemplate}></Column>
        ...
        <Column style={{ minWidth: NestedTriggerGridColumns.objectType.width }} field={NestedTriggerGridColumns.objectType.field} header={NestedTriggerGridColumns.objectType.label} sortable></Column>
        <Column
      ...
      <Paging count={totalCount} disabled={!rows.length} pageSize={pageSize} currentPage={currentPage} onChangePage={onChangeCurrentPage} onChangePageSize={onChangePageSize} />
      {loading && (
